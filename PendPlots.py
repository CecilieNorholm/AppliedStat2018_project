...
#timer_dat=timer_R1
    ...
    ax_test.text(1,71,'Fit Results:', fontsize=18, color='r', fontstyle='italic')#, horizontalalignment='center')
    ax_test.text(1,66,r'Period: 3.32 $\pm$ 0.07 s', fontsize=22, color='k', fontweight='bold')#, horizontalalignment='center')
    ax_test.text(14,77,'Time measurements [s]', fontsize=18, color='k', fontstyle='italic')
    
    ax_test.yaxis.set_minor_locator(minorloc)
    #ax_test.xaxis.set_minor_locator(minorloc)
    ax_test.tick_params(which='major', direction='in', length=7, width=2, labelsize=16)
    ax_test.tick_params(which='minor', direction='in', length=4, width=2, labelsize=12)
    ax_test.set_xlabel('Number of measurements', fontsize=14, fontweight='bold')
    ax_test.set_ylabel('Elapsed time [s]', fontsize=14, fontweight='bold')
    ...
    ax_test2.tick_params(which='major', direction='in', length=7, width=2, labelsize=12)
    ax_test2.tick_params(which='minor', direction='in', length=4, width=2, labelsize=10)
    ax_test2.set_ylabel('Fit residuals [s]', fontsize=14, fontweight='bold')
    ax_test2.text(17.5,0.23,'Time residuals [s]', fontsize=18, fontstyle='italic', fontweight='bold', color='xkcd:royal blue')
    ax_test2.text(17.5,0.18,r'Dashed lines show $\pm$ 1$\sigma$', fontsize=14, fontstyle='italic', color='k')
    
    plt.savefig('Project_Pendulum_Period_Niceplot.png')
    return 
